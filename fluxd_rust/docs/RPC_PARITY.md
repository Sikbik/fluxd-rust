...
- fundrawtransaction - Partial (wallet funding selects spendable P2PKH and P2SH (multisig) UTXOs; supports `minconf`; preserves existing `scriptSig` sizes for fee estimation; fee selection matches `fluxd` wallet: uses `paytxfee` when set, otherwise a ~2-block estimator target with a hard-coded fallback; clamps to a max fee; unsigned P2SH inputs require wallet-known redeem scripts; other non-P2PKH inputs must be pre-signed)
