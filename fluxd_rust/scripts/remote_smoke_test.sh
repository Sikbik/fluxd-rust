...
rpc_get "help" | python3 -c 'import json,sys; obj=json.load(sys.stdin); methods=obj.get("result", []) or []; need=("createp2shstarttx","signp2shstarttx","sendp2shstarttx"); missing=[m for m in need if m not in methods]; assert not missing, f"missing methods: {missing}"'
