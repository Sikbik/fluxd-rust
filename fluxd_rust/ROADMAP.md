...
- [x] [P1] Testing: document + run ignored shielded vectors suite on VPS (`scripts/run_shielded_tests.sh`) (owner: TBD)
...
- [x] [P1] Block indexing throughput parity (I/O batching, memtable tuning, UTXO cache policy) (owner: TBD)
  ...
  - [x] Inline small values in `WriteBatch` (avoid Vec allocs for fixed-size values like 32-byte hashes)
...
- [x] [P1] RPC parity expansion (see detailed checklist below) (owner: TBD)
  ...
  - [x] `estimatepriority` (confirmation-based C++ policy estimator; returns -1.0 when insufficient data is available)
...
- [x] [P1] RPC parity polish (partial -> implemented) (owner: TBD)
  ...
  - [x] `signrawtransaction` script type parity (P2SH/multisig; supports `prevtxs[].redeemScript`)
